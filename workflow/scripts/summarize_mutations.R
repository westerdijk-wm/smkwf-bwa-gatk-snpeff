...
# ordered by residue position (ascending) rather than table row order, e.g.
# "L130W/F136S" instead of an arbitrary "F136S/L130W".
...
process_batch <- function(annotated_table, use_gene_mapping, target_gene_ids) {
  ...
  list(summary = mutation_summary, samples = all_samples_batch, genes = genes_observed_batch)
}
...
if (use_gene_mapping) {
  ...
  # observed variants anywhere, so they still show up as "Wildtype".
...
final_long <- full_grid %>%
  ...
  mutate(Final_mutation = replace_na(Final_mutation, "Wildtype")) %>%
